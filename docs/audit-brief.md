...
- **A platform limit found the same way, unrelated to the spawn path itself: Windows has no OS-level SIGTERM.** `cli-run.mjs`'s graceful shutdown (`process.on('SIGTERM', ...)`, kill the lane's process group, then exit 143) is a POSIX guarantee only: `ChildProcess.kill('SIGTERM')` on Windows calls `TerminateProcess()` unconditionally, giving the target process no chance to run any handler at all, proven on `windows-latest` CI (the wrapper died as `{code: null, signal: 'SIGTERM'}`). SIGINT is different and unaffected: Windows delivers it as a real, catchable console-control event, so the same graceful path is expected to hold for it on every OS, and `test/cli.test.js`'s `#13` still asserts that.
