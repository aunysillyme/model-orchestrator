...
- **`test/cli.test.js`'s fake lane binaries were `#!/bin/sh` scripts, which Windows cannot execute as `argv[0]`** (no shebang interpretation in `CreateProcess`). `writeShellStub()`/`writeNodeStub()` now write a tiny `.cmd` launcher beside the POSIX file on win32, handing off to Git Bash's `sh.exe` (already reachable, located from this file's own `PATH`, since the CI job's shell is bash) or `node`, so every existing test body runs unchanged on every OS and `cli-run.mjs`'s own spawn/timeout/signal/process-group handling is exercised for real rather than skipped.
