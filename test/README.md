...
| `plugin.test.js` | the Claude Code plugin in `plugin/` is exactly what `npm run gen:plugin` generates, every `${CLAUDE_PLUGIN_ROOT}` reference resolves, its hooks make no network call, write, subprocess or credential read, every agent declares `tools:`, the README carries `/plugin install`, and the plugin's `route-gate.mjs` finds the default rules, reports a non-file, and names `npx model-orchestrator` when there are none. Each check has a case proving it can go red. |
